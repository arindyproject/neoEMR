        ...
        <div class="col-lg-9 col-xl-9 col-md-9 col-sm-12">
            <div class="card">
                <div class="card-header bg-{{$bg}}">
                    <h3 class="card-title">
                        <i class="fas fa-id-badge"></i> Template Profil Pasien
                    </h3>
                </div>

                <div class="card-body">
                    <form action="{{Route('administration.setting.print.pasien.profil.store')}}" method="POST">
                        @csrf
...
<script>
    ...
    @foreach($list as $i=>$file)
    PDFObject.embed("{{Route('print.patient.profil',[1, str_replace(".blade.php","",$file->getFilename())])}}", "#pdf-{{ $i }}", options);
    @endforeach
