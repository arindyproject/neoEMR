...
class AlamatProvinsiController extends Controller
{
    public function __construct()
    {
        ...
        $this->paging       = 30;
        $this->view_index   = 'attributes.alamat.provinsi.index';
        $this->view_edit    = 'attributes.alamat.provinsi.edit';

        $this->url_index    = 'attributes.alamat.provinsi.index';
        $this->url_edit     = 'attributes.alamat.provinsi.edit';
        $this->url_store    = 'attributes.alamat.provinsi.store';
        $this->url_update   = 'attributes.alamat.provinsi.update';
        $this->url_delete   = 'attributes.alamat.provinsi.destroy';

        $this->title        = "ATT Alamat Provinsi";
        $this->table        = new attAlamatProvinsi;
        $this->to_return    = [
            'title'     => $this->title,
    ...
    {
        $id = request('id');
        $this->to_return['data'] = $this->table::
    ...
    public function store(Request $request){
        $request->validate([
            ...
            'kode' => "nullable|string|max:255|unique:att_alamat_provinsis",
        ]);
    ...
    public function update(Request $request, $id){
        $request->validate([
            'nama' => "required|string|max:255",
            'kode' => "nullable|string|max:255|unique:att_alamat_provinsis,kode," . $id .',id',
        ]);
    ...
    public function autocomplete(Request $request)
    {
        $data = [];
        if($request->filled('q')){
            $data = $this->table::select("kode","nama", "id")->where('nama', 'LIKE', '%'. $request->get('q'). '%')->get();
        }
